...
def check_key(api_key):
    if api_key not in KEYS_DB:
        return None
    if KEYS_DB[api_key]["status"] != "active":
        return "revoked"
    if datetime.now() > datetime.fromisoformat(KEYS_DB[api_key]["expires"]):
        return "expired"
...
def get_user():
    ...
    try:
        r = requests.get(
            f"https://users-xinfo-admin.vercel.app/api?key=demo&type=user&term={term}",
...
def get_mobile():
    ...
    try:
        r = requests.get(
            f"https://users-xinfo-admin.vercel.app/api?key=demo&type=mobile&term={term}",
            headers={"User-Agent": "Mozilla/5.0"},
            timeout=10
        )
        data = r.json()
